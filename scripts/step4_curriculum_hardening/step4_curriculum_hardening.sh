...
#SBATCH --job-name step4_hardening
...
STEP_NAME="step4_curriculum_hardening"
...
RUN_NAME="${RUN_NAME:-step4_hardening_${TIMESTAMP}}"
TRAIN_LOG_DIR="${DIGI_ROOT}/logs/${RUN_NAME}"
STEP_LOG="${JOB_LOG_DIR}/${STEP_NAME}-${TIMESTAMP}.log"

if [[ -z "${RESUME_MODEL}" ]]; then
    echo "RESUME_MODEL is required. Example:"
    echo "  sbatch --export=RESUME_MODEL=logs/<step2_run>/tqc_drilling_final.zip ${BASH_SOURCE[0]}"
    exit 1
fi
